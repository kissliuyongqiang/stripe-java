...
public class HttpURLConnectionClient extends HttpClient {
  ...
  private Class<URLStreamHandler> customURLStreamHandler;

  public HttpURLConnectionClient() {
    this(null);
  }

  public HttpURLConnectionClient(Class<URLStreamHandler> customURLStreamHandler) {
    this.customURLStreamHandler = customURLStreamHandler;
  }
  ...
  private StripeResponse getStripeResponse(
    ...
    return this.makeUrlConnectionRequest(method, url, query, options);
  }

  private StripeResponse getMultipartStripeResponse(
      ApiResource.RequestMethod method,
      String url,
      Map<String, Object> params,
      RequestOptions options)
    ...
    try {
      conn = this.createStripeConnection(url, options);
  ...
  private HttpURLConnection createStripeConnection(String url, RequestOptions options)
      throws IOException {
    final URL stripeUrl = this.createStripeUrl(url);
  ...
  private URL createStripeUrl(String url) throws IOException {
    URL stripeUrl;

    if (this.customURLStreamHandler != null) {
      // instantiate the custom handler provided
      URLStreamHandler customHandler;
      try {
        Constructor<URLStreamHandler> constructor = this.customURLStreamHandler.getConstructor();
        customHandler = constructor.newInstance();
  ...
  private HttpURLConnection createGetConnection(String url, String query, RequestOptions options)
      throws IOException {
    String getUrl = formatUrl(url, query);
    HttpURLConnection conn = this.createStripeConnection(getUrl, options);
    conn.setRequestMethod("GET");

    return conn;
  }

  private HttpURLConnection createPostConnection(String url, String query, RequestOptions options)
      throws IOException {
  ...
  private HttpURLConnection createDeleteConnection(String url, String query, RequestOptions options)
      throws IOException {
    String deleteUrl = formatUrl(url, query);
    HttpURLConnection conn = this.createStripeConnection(deleteUrl, options);
  ...
  private StripeResponse makeUrlConnectionRequest(
    ...
    try {
      switch (method) {
        case GET:
          conn = this.createGetConnection(url, query, options);
          break;
        case POST:
          conn = this.createPostConnection(url, query, options);
          break;
        case DELETE:
          conn = this.createDeleteConnection(url, query, options);
          break;
        default:
          throw new ApiConnectionException(
              String.format(
